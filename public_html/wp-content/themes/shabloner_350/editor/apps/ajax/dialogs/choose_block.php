...
<?php foreach($cats as $int => $cat) { ?>
  <a data-cat_id="<?=$cat['id']?>" class="nav-link <?php if($int==0) echo 'active';?>" id="v-pills-<?=$cat['id']?>-tab" data-toggle="pill" href="#v-pills-<?=$cat['id']?>" role="tab" aria-controls="v-pills-<?=$cat['id']?>" aria-selected="<?php if($int==0) echo 'true';?>">
...
<?php foreach($cats as $int => $cat) { ?>
  <div class="tab-pane fade show <?php if($int==0) echo 'active';?>" id="v-pills-<?=$cat['id']?>" role="tabpanel" aria-labelledby="v-pills-<?=$cat['id']?>-tab">
...
function block_add_to_area(block_id, block_schema_id)
{
	...
	.done(function( data ) {
		...
		.done(function( data ) {
			$('#block_'+block_schema_id).after(data);
			$('#block_'+block_schema_id).remove();
		});	
	});	
	
}

function block_add_exists_to_area(block_schema_id, block_schema_replace_id, no_replace)
{
	...
	.done(function( data ) {
		if(data.block_id)
		{
			if(no_replace == 1) $('#area_blocks').append('<div id="block_'+block_schema_id+'"></div>');
			$.ajax({
				type: "GET",
				url: '/ajax/app',
				data: {
			...
			.done(function( data ) {
				if(no_replace != 1)
				{
					$('#area_blocks #block_'+block_schema_replace_id).after(data);
					$('#area_blocks #block_'+block_schema_replace_id).remove();
				}
				else
				{
					$('#block_'+block_schema_id).after(data);
		...
		$('#cat_block_'+cat_id).html(data);
			...
			$('.block_id').bind('click', function() {
				...
				.done(function( data ) {
					...
					if(data.id) 
					{
						$('#area_blocks').append('<div id="block_'+data.id+'"></div>');
...
else echo "show_blocks_by_cat_id(1);"; ?>  
