...
require get_template_directory() . '/editor/includes/functions.php';
if(!stristr(get_option('home'), $_SERVER['HTTP_HOST'])) redirect(get_option('home')); 

function numWithSpaces($number,$minus=0)
{
...
if(function_exists('register_nav_menus')){
	register_nav_menus(
		array( // создаём любое количество областей
		  'menu_main' => 'Главное меню', // 'имя' => 'описание'
		  'menu_second' => 'Второе меню',
		  'menu_third' => 'Третье меню',
		  'menu_fourth' => 'Четвертое меню',
		)
	);
}
...
function showSocials($template='')
{
	...
	$socials['vk'] = get_option('prdvg_vk');
	$socials['ok'] = get_option('prdvg_ok');
	$socials['instagram'] = get_option('prdvg_instagram');
	$socials['facebook'] = get_option('prdvg_facebook');
	$socials['youtube'] = get_option('prdvg_youtube');
	$socials['twitter'] = get_option('prdvg_twitter');
